...
Package menura is designed to fit stochastic differential equation (diffusion) models to continuous trait data on a phylogeny. It uses a Data Augmentation and Markov Chain Monte Carlo (DA-MCMC) algorithm to obtain estimates of posterior distributions of the model parameters, conditional on the observed data (ie the trait values). menura was designed to study the behaviour and estimation of Gaussian and Non-Gaussian diffusion models on phylogenies. These models are difficult to fit because they attempt to describe the dynamics of traits through macroevolutionary deep time, yet typically we only have trait values at the tips of the tree. Hence, in the absence of a good fossil record there is only a small amount of information in the data about the evolutionary process. Thus menura is EXPERIMENTAL SOFTWARE. USE AT YOUR OWN RISK. In particular, run multiple, long MCMC chains, with different starting values. Use convergence diagnostics and plot the traces to check for convergence. Use weakly informative priors, strong enough to regularise the problem but still weak enough to be overwhelmed by the data. For the mathematical background and underlying statistics of package menura, see @blomberg2017.
...
##Description:
  ...
  $$dX_t = b(X_t, \alpha, \mu) dt + \sigma(X_t, \epsilon) dW_t,~~X_0 = x_0$$
  where $X_t$ denotes the state variable, $t$ the time,
  $b$ the drift function and $\sigma$
 the diffusion function are known
  in parametric from which
  $\alpha$, $\mu$, and $\epsilon$
  are the parameters, and $W_t$ is Brownian motion.
  The value of $X_t$ at time $t_0$, $X_0$,
  is independent of $W_t$. Note that below, when $\sigma$ is a constant function, we let $\epsilon = \sigma$.
...
Specify the diffusion process as
   ...
   **Cox-Ingersoll-Ross (CIR)** model is given by
    $$dXt = \alpha (\mu - X_t) dt + ~\sqrt(\epsilon~ X_t) dWt$$
   with  $X_0 = x_0 > 0$, where Wt is the Brownian process, alpha, mu,
   and $\epsilon$ are the model parameters which are all
   ...
   **Beta** model is given by
    $$dX_t = \alpha(\mu - X_t) dt + \sqrt(\epsilon~X_t (1-X_t)) dW_t,$$
   with  $X_0 = x_0 > 0$,  $W_t$ is the Brownian process,
  alpha, mu, and sigma are the model parameters
  where alpha and sigma are positive values.
